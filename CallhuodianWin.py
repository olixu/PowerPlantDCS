...
class MainWindow(QMainWindow):
	...
	def OpenFile(self):
		self.filename = QFileDialog.getOpenFileName(self, 'open file', '/media/boss/大学资料/大学4年学习资料/大四下/毕业设计所有资料/嘉兴数据/完整版/20160612-0624')
	...
	def Calculate(self):
		##plant_data.drop([1], inplace=True)
		raw_给煤量 = ['给煤机A给煤量','给煤机B给煤量','给煤机C给煤量','给煤机D给煤量','给煤机E给煤量','给煤机F给煤量']
		raw_主蒸汽压力 = ['主蒸汽压力']
		raw_炉膛出口烟温度 = ['炉膛烟温探针B']
		...
		raw_二次风温 = ['二次风温度']
		raw_含氧量 = ['省煤器A侧出口烟气氧量分析1','省煤器 A侧出口烟气氧量分析2','省煤器A侧出口烟气氧量分析3','省煤器B侧出口烟气氧量分析1','省煤器B侧出口烟气氧量分析2','省煤器B侧出口烟气氧量分析3']
		...
		fields = [给煤量, 主蒸汽压力, 炉膛出口烟温度, 主蒸汽温度, 机组负荷, 给水温度, 给水流量, 一次风量, 二次风量, 二次风温, 含氧量]
		outcome = pd.concat(fields, axis=1)
		outcome.columns = ['给煤量', '主蒸汽压力', '炉膛出口烟温度', '主蒸汽温度', '机组负荷', '给水温度', '给水流量','一次风量', '二次风量', '二次风温', '含氧量']
		...
		self.ui.DataMiningTableWidget.setHorizontalHeaderLabels(['给煤量', '主蒸汽压力', '炉膛出口烟温度', '主蒸汽温度', '机组负荷', '给水温度', '给水流量','一次风量', '二次风量', '二次风温', '含氧量'])
		self.ui.DataMiningTableWidget.setVerticalHeaderLabels(['count', 'mean', 'std', 'min', '25%', '50%', '75%','max'])
		for i in range(self.df_rows):
			for j in range(self.df_cols):
				x = '{:.3f}'.format(ruanceliang_data.iloc[i, j])
				self.ui.DataMiningTableWidget.setItem(i, j, QTableWidgetItem(x))
